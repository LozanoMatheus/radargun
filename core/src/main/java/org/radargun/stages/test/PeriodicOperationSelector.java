...
public class PeriodicOperationSelector implements OperationSelector {
   ...
   public Operation next(Random random) {
      ...
      if (record == null) {
         record = new Record(new Random(Thread.currentThread().getId() ^ System.nanoTime()).nextLong() % requestPeriod);
         threadLocalRecord.set(record);
      }
      long waitTime = TimeUnit.NANOSECONDS.toMillis(startTime + record.rampUp + requestPeriod * record.opNumber - System.nanoTime());
