...
public class RemoteSlaveConnection {
   ...
   private static class SlaveRecord {
      int index; // slave id, should be equal to index in slaves field
      UUID uuid; // key unique for given series of generations of this slave
   ...
   private int connectSlaves() throws IOException {
      ...
      while (it.hasNext()) {
         ...
         ByteBuffer uuidBytes = readBytes(socketChannel, 16);
   ...
   private void readResponse(SelectionKey key) throws IOException {
      ...
      if (byteBuffer.position() >= 4) {
         ...
         if ((expectedSize + 20) > byteBuffer.capacity()) {
            ByteBuffer replacer = ByteBuffer.allocate(expectedSize + 20);
         ...
         if (byteBuffer.position() >= expectedSize + 20) {
            ...
            long uuidMsb = byteBuffer.getLong(4 + expectedSize);
            long uuidLsb = byteBuffer.getLong(4 + expectedSize + 8);
